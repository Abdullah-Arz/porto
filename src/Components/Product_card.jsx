...
function Product_card({id,data,breadcumb,cattocart,NavigateToDetailCart}) {

  console.log('Product_card ---- ', data)
  // console.log('image id ---- ', data.images[1])
...
const Navigate_To_Categories = () => {
  ...
  if(NavigateToDetailCart) { 
    navigate(`/categories/singleproduct/${data.id}`,{state: {data}})
  }
  else if(cattocart){
    ...
    console.log('Rating ---- ',data.reviews)
      ...
      <div  className='product-card'>
            ...
            >
               <div className="d-flex card_top justify-content-between p-3 mt-1">
                ...
                <MDBIcon
                onClick={()=>{SelectCart(data.id,data.name,data.price,data.images[0] ? data.images[0].image_path : null)}} 
